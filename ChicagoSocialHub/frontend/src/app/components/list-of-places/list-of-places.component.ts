...
export class ListOfPlacesComponent implements OnInit {
  ...
  findStations(placeName) {
    ...
    this.placesService.findStations(placeName).subscribe(() => {
      this.router.navigate(['/list_of_stations/google_map_marker']);
    });

  }

  findAllStationsNearby() {
    this.placesService.findAllStationsNearbyPlaces().subscribe(() => {
      this.router.navigate(['/list_of_places/heat_map_divvy_stations']);
    });
  }
